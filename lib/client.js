...
Client.prototype.getMORefProperties = function( MORef, propertyArray ) {
  ...
  this.runCommand('CreateContainerView', { _this: viewManager,
    ...
    .once('result', function(result) {
      ...
      this.runCommand('RetrievePropertiesEx', { _this: propertyCollector, specSet: [ propertyFilterSpec ], options: {} })
        .once('result', function(result){
          emitter.emit('result', result);
        })
        .once('error', function(err){
            ...
            if( typeof result == 'undefined' || _.isEmpty(result)) {
              if(processedCount === templateFolders.length) {
                if(vAppArray.length === 0) {
                  emitter.emit('error', 'No vApps found in template folder with name \'' + folderName + '\'');
                }
...
*/
Client.prototype.cloneVirtualAppliance = function( 
                      ...
                      self.waitForValues( taskMORef , ['info.state','info.error'], 'state', ['success','error'])
                        .once('result', function(result) {
                          ...
                          if( result['info.error'] == undefined ) {
                            ...
                            /*
                            emitter.emit('message', 
                              'Clone ' + srcVirtualAppName + ': Updating network adapters' );
                            
                            // Update Network Adapters
                            updateNetworkAdapters( vAppName, studentId )
                              .once('ready', function() {
                                emitter.emit('message', 
                                  'Clone ' + srcVirtualAppName + ': Network adapters updated' );
                                emitter.emit('result', vAppName);
                              })
